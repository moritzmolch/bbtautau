...
def get_hist_backgrounds_total(
    hist_backgrounds: list[ROOT.TH1]
):
...
def fit_hist_ratio(
    ...
    x, y, y_err = (
        [hist_ratio.GetXaxis().GetBinCenter(i) for i in range(1, hist_ratio.GetNbinsX() + 1)],
        [hist_ratio.GetBinContent(i) for i in range(1, hist_ratio.GetNbinsX() + 1)],
        [hist_ratio.GetBinError(i) for i in range(1, hist_ratio.GetNbinsX() + 1)],
    )
...
def set_axes_limits_distributions(
    ax: mpl.axes.Axes,
    hists: Optional[ROOT.TH1] = None,
    custom_y_limits: Optional[Tuple[float, float]] = None,
):
    ...
    if custom_y_limits is not None:
        ...
        y_maxs = np.array(
            [
                hist.GetMaximum() + np.sqrt(hist.GetMaximum())
                for hist in hists
            ],
        )
...
def plot(
    hist_data: Tuple[ROOT.TH1, Dict[str, Any]],
    hist_backgrounds: List[Tuple[ROOT.TH1, Dict[str, Any]]],
    hist_signals: Optional[List[Tuple[ROOT.TH1, Dict[str, Any]]]] = None,
    stack_kwargs: Optional[Dict[str, Any]] = None,
    x_label_top: Optional[str] = None,
    y_label_top: Optional[str] = None,
    x_label_bottom: Optional[str] = None,
    y_label_bottom: Optional[str] = None,
    y_limits_top: Optional[Tuple[float, float]] = None,
    y_limits_bottom: Optional[Tuple[float, float]] = None,
    ...
    for hist_signal in hist_signals:
        ...
        h = c * hist_signal[0]
        plot_distribution(
            ax_top, h, yerr=False, linewidth=3, **plot_kwargs
        )
    ...
    set_axes_limits_distributions(
        ax_top,
        [hist_data[0], hist_backgrounds_total],
        custom_y_limits=y_limits_top,
    )
    set_axes_limits_ratio(
        ax_bottom,
        [hist_data_ratio, hist_backgrounds_total_ratio],
